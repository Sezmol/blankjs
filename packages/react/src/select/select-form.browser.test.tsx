...
const renderControl = (selected = false) => (
  <Select.Root name="choice" defaultValue={selected ? "a" : undefined} required>
...
test("trigger respects canceled click", async () => {
  render(
    <Select.Root>
      <Select.Trigger onClick={e => e.preventDefault()}>Choose</Select.Trigger>
    </Select.Root>,
  );
...
test("canceled keyboard leaves the list closed, normal click opens it", async () => {
  render(
    <Select.Root>
      <Select.Trigger onKeyDown={e => e.preventDefault()}>Choose</Select.Trigger>
    </Select.Root>,
  );
